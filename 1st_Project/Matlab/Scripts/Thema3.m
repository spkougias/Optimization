...
for l_idx = 1:length(l_values)
    [~,n_values(1,l_idx)] = Fibonacci_method(f1, a, b, e_fixed, l_values(l_idx));
    [~,n_values(2,l_idx)] = Fibonacci_method(f2, a, b, e_fixed, l_values(l_idx));
    [~,n_values(3,l_idx)] = Fibonacci_method(f3, a, b, e_fixed, l_values(l_idx));
end

figure;
sgtitle(['Fibonacci: Number of Evaluations vs. l for \epsilon = ', num2str(e_fixed, '%.3f')]);
...
for l_idx = 1:length(l_vector)
    [intervals_f1{l_idx}, ~] = Fibonacci_method(f1, a, b, e_fixed, l_vector(l_idx));
    [intervals_f2{l_idx}, ~] = Fibonacci_method(f2, a, b, e_fixed, l_vector(l_idx));
    [intervals_f3{l_idx}, ~] = Fibonacci_method(f3, a, b, e_fixed, l_vector(l_idx));
end

figure;
sgtitle(['Fibonacci: Interval Endpoints vs. Iteration k for \epsilon = ', num2str(e_fixed, '%.3f')]);
