function [intervals, eval_num] = bisection_method_with_derv(df, a_initial, b_initial, l)
    ...
    while ~(intervals(end,2) - intervals(end,1) < l)
        ...
        % Check if midpoint is close enough to zero
        if (intervals(end,2) - intervals(end,1)) / 2 < 1e-10
           break; 
        end
