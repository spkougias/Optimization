...
% Strategy : {name, mode , gamma initial/range}
strategies = {
    'Fixed Step (gamma=0.1)', 1, 0.1;
    'Exact Search (Bisection)', 3, 2.0;
    'Armijo Rule', 2, 1
};
...
for i = 1:size(start_points, 1)
    ...
    for s = 1:size(strategies, 1)
        ...
        [x_hist, iters] = steepest_descent(f_sym, vars, x0, epsilon, gamma_val, mode);
        ...
        fig_name = sprintf('Start (%.0f, %.0f) - %s', x0(1), x0(2), strat_name);
        ...
        plot(x0(1), x0(2), 'gx', 'LineWidth', 2, 'MarkerSize', 10); % Start (green X)
        plot(x_hist(end,1), x_hist(end,2), 'ro', 'LineWidth', 2, 'MarkerSize', 10); % End (red O)
